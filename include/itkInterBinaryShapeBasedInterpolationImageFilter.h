...
#ifndef __INTERBINARYSHAPEBASEDINTERPOLATIONIMAGEFILTER_H
#define __INTERBINARYSHAPEBASEDINTERPOLATIONIMAGEFILTER_H
...
namespace itk
{
  ...
  * \brief Interpolates  a 3D segmented binary image wherein only a few regularly spaced slices have been segmented. The missing contours are estimated.
  * This filter takes as input a 3D binary image wherein the object of interest is represented 
  * only on certain slices that are regularly spaced. It yields a 3D binary image of the same size as 
  * that of the input and wherein the object is represented on all contiguous slices that comprise it.
  * Note that the DelineationRatio defined as the ratio of the delineated slices in the output image 
  * (i.e. after interpolation) to the number of the delineated slices in the input image (i.e. before 
  * interpolation) needs to be set up.
  * Reference paper : S.P. Raya and J.K. Udupa. Shape-based interpolation of multidimensional objects. 1990
  */
  ...
  public:           
    ...
    /** Standard class typedefs used for object creation with the object factory. */
     ...
     * the input image (i.e. before interpolation). */
  ...
  protected:
    ...
    // Find all slices that are delineated (i.e. wherein there are non-zero pixel intensities). 
    // They are not contiguous in the input image.
    // The distance map calculation will be performed on these slices only to gain time of calculation.
    void FindZCoordinatesOfDelineatedSlices();

    // Generate the information (origin, spacing, size, LagestPossibleRegion,...) of the image made of the delineated slices only    
  ...
  private:
    //The copy constructor and assignment operator should be declared private and
    //not implemented, to prevent instantiating the ﬁlter without the factory methods
