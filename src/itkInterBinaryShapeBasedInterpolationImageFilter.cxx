...
{  
  ...
  void InterBinaryShapeBasedInterpolationImageFilter::FindZCoordinatesOfDelineatedSlices()
  {
    ...
    // Count the number of slices that have been delineated in the input image and store their z coordinate in a vector 
    // using the STL library (std::vector)
  ...
  void InterBinaryShapeBasedInterpolationImageFilter::GenerateData()
  {
    ...
    for( itr.GoToBegin(), outItr.GoToBegin(); !itr.IsAtEnd(); ++itr, ++outItr )
    {
      if( itr.Get() > 0 ) 
      {
	if( floor(10*itr.Get())/10 > 0) outItr.Set(0);
	else outItr.Set(1);
      }
      else outItr.Set(1);
    }
